...
This project presents an end-to-end computer vision pipeline designed to automate both the census and demographic analysis of seal colonies. Using the **Wildlife Aerial Image Dataset (WAID)**, a fine-tuned **YOLOv8** model is employed to robustly detect and count seals against complex backgrounds containing diverse animal classes. Furthermore, an unsupervised clustering approach is introduced to estimate colony structure. By integrating geometric morphometrics (normalized area, rotation-invariant elongation, pelage texture), social proximity features, and semantic embeddings from **CLIP**, male "bull" seals are successfully isolated from the general population. This framework offers a scalable, non-invasive tool for monitoring breeding colony dynamics without the need for expensive expert annotation.
